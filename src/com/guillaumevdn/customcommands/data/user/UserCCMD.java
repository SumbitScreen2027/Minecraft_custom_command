...
public final class UserCCMD {
	...
	public UserCCMD(UUID playerUUID, RWLowerCaseHashMap<Long> lastUses, RWLowerCaseHashMap<Boolean> toggles) {
		...
		this.ref = new KeyReference<>(playerUUID);
	...
	public void setToSave() {
		BoardUsersCCMD.inst().addCachedToSave(ref);
	}
	...
	public static void process(UUID key, Consumer<UserCCMD> processor) { BoardUsersCCMD.inst().fetchValue(key, user -> { processor.accept(user); if (Bukkit.getPlayer(key) == null) { BoardUsersCCMD.inst().disposeCacheElements(BukkitThread.ASYNC, CollectionUtils.asSet(KeyReference.of(key)), null); } }, null, false, true); }
